...
const Result = (props) => {
  ...
  const submitForm = () => {
    const blob = new Blob([JSON.stringify(state.ase1Preflight, null, "\t")], {
      "Content-Type": "application/json" + JSON.stringify(state.smartWaypointState, null, "\t"),
    });
    ...
    console.log("Listening:", state.smartWaypointState);
  };

  return (
    <div className="container">
      <pre id="results">{JSON.stringify(state.smartWaypointState, null, 2)}</pre>
